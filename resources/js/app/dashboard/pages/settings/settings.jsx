...
const Settings = () => {
    ...
    return (
    <div className='h-full bg-gray-200 text-slate-800 dark:text-gray-300 dark:bg-main-dark-bg rounded-xl overflow-auto flex flex-col gap-1'>
        ...
        <div className='flex-1 flex flex-col md:flex-row gap-1'>
            <div className='w-full md:w-[200px] pt-2 bg-gray-50 dark:bg-secondary-dark-bg'>
                <div className='flex md:block flex-wrap gap-3 md:h-full overflow-auto'>
                    {SettingLists.map((list, i)=>(
                        <NavLink to={`/dashboard/modal/settings/${list.title.replace(' ','-')}`}
                            key={i}
                            ...
                            <span className='text-xl text-gray-800 dark:text-slate-300'>{list.icon}</span>
                            <span className='ml-2 text-xs font-bold capitalize'>{list.title}</span>
                        </NavLink>
                    ))}   
                </div>
            </div>
            <div className='w-full md:flex-1 h-full overflow-auto px-5 text-slate 
