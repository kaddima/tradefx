...
const Sidebar = () => {
	...
	const activeLinkClass = `flex items-centers gap-5 pl-4 pt-3 pb-2.5
	rounded-lg text-white text-md m-2`

	const unActiveLinkClass = `flex items-centers gap-5 pl-4 pt-3 pb-2.5 rounded-lg
	text-md text-grey-700 dark:text-gray-200 dark:hover:text-black hover:bg-light-gray m-2`
					...
					{links.map((link, i)=>(
						<div key={i} className="relative">
							...
							>
								...
								{(link.name.toLowerCase() == 'charts' && chartsCounts > 0) && (<div className='absolute bottom-6 right-5 text-sm text-extrabold 
								bg-slate-800 w-4 h-4 rounded-full flex justify-center items-center' style={{color:currentColor}}>
